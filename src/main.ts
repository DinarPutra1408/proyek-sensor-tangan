...
// --- 1. CONFIGURATION (Gunakan 'let' agar bisa dimodifikasi dat.gui) ---
...
// --- 3. RUMUS BENTUK PREMIUM (FIXED LOVE & BIG FLOWER) ---
function generateShapePositions(shapeIndex: number) {
    ...
    for (let i = 0; i < PARTICLE_COUNT; i++) {
        ...
        if (shapeIndex === 1) { 
            ...
            colors[i3] = 1.0; colors[i3+1] = 0.2; colors[i3+2] = 0.4; // Pinkish Red
        } 
        else if (shapeIndex === 2) { 
            // --- 2 JARI: BUNGA RIMBUN (REFERENSI GAMBAR 2) ---
            const petals = 8;
            const flowerSize = SHAPE_SCALE * 1.3; // Dibuat lebih besar dari yang lain
            const r = Math.abs(Math.cos(petals / 2 * t)) * flowerSize * r_rand;
            
            x = r * Math.cos(t);
            y = r * Math.sin(t);
            // Efek dimensi pada kelopak
            z = Math.sin(r * 1.5) * 0.6 + (Math.random() - 0.5) * 0.2;
            
            colors[i3] = 0.4; colors[i3+1] = 0.5; colors[i3+2] = 1.0; // Biru Ungu Mewah
        }
        else if (shapeIndex === 3) { 
            // --- 3 JARI: BINTANG TAJAM ---
            const spikes = 5;
            ...
            currentR *= r_rand;
            x = Math.cos(t) * currentR;
            y = Math.sin(t) * currentR;
            z = (Math.random() - 0.5) * 0.3;
            
            colors[i3] = 0.2; colors[i3+1] = 0.9; colors[i3+2] = 1.0; // Cyan
        }
        else { 
            // --- STANDBY: GALAXY NEBULA ---
...
hands.onResults((res: Results) => {
    if (res.multiHandLandmarks?.length) {
        ...
        // Rotasi mengikuti posisi pergelangan tangan
        rotY = (lm[0].x - 0.5) * 5;
        rotX = (lm[0].y - 0.5) * 5;
...
function animate() {
    ...
    points.rotation.z += 0.003; // Slow spin
